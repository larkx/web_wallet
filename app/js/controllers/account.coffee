angular.module("app").controller "AccountController", ($scope, $state, $filter, $location, $stateParams, $q, Growl, Wallet, Utils, WalletAPI, $modal, Blockchain, BlockchainAPI, Info, Observer, $translate) ->
    ...
    $scope.wallet_info = {file: "", password: "", type: 'CopperLark/Bitcoin/Old PTS'}
...
#        Wallet.refresh_account(name)
    ...
    $scope.import_wallet = ->
        ...
        switch $scope.wallet_info.type
            when 'CopperLark/Bitcoin/Old PTS' then promise = WalletAPI.import_bitcoin($scope.wallet_info.file,$scope.wallet_info.password,$scope.account.name)
        ...
        promise?.then (response) ->
            $scope.wallet_info.type = 'CopperLark/Bitcoin/Old PTS'
